...
```{r}
health_statements_raw <-
  as_tibble(fread("../../../../../../Sudan/Health/Health Policy Statement sudan.csv")) %>% 
  rename ("AMOUNT" = "RECEIPT_AMOUNT") %>%
...
```{r}
length(intersect(health_products$POLICY_NO, health_statements_raw$POLICY_NO))
```
## Product values
...
health_statements_raw %>% 
  ...
  # filter(TRANSACTION_TYPE == "Create Policy") %>% 
  filter(TRANSACTION_DATE >= "2018-01-01") %>% 
  left_join(health_products, by = "POLICY_NO") %>% 
  select(POLICY_TYPE, AMOUNT) %>% 
  group_by(POLICY_TYPE) %>% 
  summarise(product_value = round(mean(AMOUNT)),
            accounts = n()) %>% 
...
```{r}
product_values <- 
  bind_rows(general_product_values, health_product_values)
...
```{r}
save.image("../outputs/28June_Product_Values.RData")
# load("../outputs/23June_Product_Values.RData")
