...
health_product_holding <- readRDS("../../02. Determine product holding/outputs/health_product_holding.rds") %>% 
...
products_df_raw <-
  bind_rows(
  health_product_holding,
  general_product_holding
) %>% 
  distinct(ACCOUNT_NO, PRODUCT, .keep_all = TRUE)

unique(products_df_raw$PRODUCT)
...
save.image("../outputs/28June_RatingMatrix.RData")
