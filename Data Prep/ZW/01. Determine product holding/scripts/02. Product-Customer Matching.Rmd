...
t24_products_hashed %>%
  ...
  ungroup() %>% 
  write_csv("../../../../../../C - Documentation/Data/festus_extraction/ZW/t24_products_cleaned.csv")
```
...
post_raw %>% 
  count(OLD_NAME) %>% 
  mutate(PRODUCT = str_extract(string = str_to_lower(OLD_NAME), 
                               pattern = "ewallet|textacash"),
         PRODUCT = coalesce(PRODUCT,"Custom Card")) %>% 
  select(OLD_NAME, PRODUCT) %>% 
  filter(PRODUCT != "Custom Card")
...
# Check if banking products have been updated
banking_product_holding %>% 
  distinct(PRODUCT)
```
...
```{r}
save.image(file = "../outputs/images/08July.RData")
