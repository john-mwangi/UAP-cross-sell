...
health_products_raw %>% 
  ...
  filter (!`BENEFIT DESCRIPTION` == "NULL") %>%
  count(ACCOUNT_NO,`BENEFIT DESCRIPTION`) %>% 
  arrange(desc(n))

health_prod_holding %>%
  distinct(`BENEFIT DESCRIPTION`)
...
```{r}
gi_products_raw %>% count(POLICY_TYPE) 
```
...
# Project image
save.image(file = "./27May_Product_Cust_Matching.RData")
load("27May_Product_Cust_Matching.RData")
```
