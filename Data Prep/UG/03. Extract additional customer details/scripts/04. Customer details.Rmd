...
gi_products <-
readRDS(file = "../../product_holding/gi_product_holding_final.rds") %>% 
...
saveRDS(product_businesslines, "../../../../Models/UG/scripts/product_businesslines.rds")
...
health_product_holding_final %>% 
            select(ACCOUNT_NO) %>% 
  left_join(health_customers, by = c("ACCOUNT_NO"="ACCOUNT_NO")) %>% 
  # rename(ACCOUNT_NO = ACCOUNT_NO.y) %>% 
  select(any_of(x = customer_attributes)) %>% 
  mutate(ACCOUNT_NO = as.character(ACCOUNT_NO))%>% 
  group_by(ACCOUNT_NO) %>% 
  slice(1) %>% 
...
# Project image
save.image("./31May_Customer_details.RData")
load("../../../../Models/UG/scripts/31May_Models.RData")
```
