...
general_products %>% 
  distinct(ACCOUNT_NO,BUSINESS_TYPE) %>% 
  count(ACCOUNT_NO,BUSINESS_TYPE)
...
general_product_holding_raw %>% 
  filter(!BUSINESS_TYPE %in% insufficent_products)
...
gibbs_raw %>% 
  select(ACCOUNT_NUMBER,CONTRACT_GROUP_NAME) %>% 
  mutate(system = "gibbs") %>% 
  rbind(
    oipa_raw %>% 
      select(ACCOUNT_NUMBER,CONTRACT_GROUP_NAME) %>% 
      mutate(system = "oipa")) %>% 
  distinct(ACCOUNT_NUMBER, CONTRACT_GROUP_NAME, .keep_all = TRUE) %>% 
  count(ACCOUNT_NUMBER, CONTRACT_GROUP_NAME) %>% 
  filter(!is.na(ACCOUNT_NUMBER) & !is.na(CONTRACT_GROUP_NAME))
