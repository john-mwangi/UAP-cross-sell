...
```{r}
saveRDS(health_product_holding_final, file = "./health_product_holding_final.rds")
```
...
Do account numbers under products and customer master match? Yes, all of them match.

```{r}
length(unique(asset_product_holding$ACCOUNT_NO))

length(
  intersect(unique(asset_product_holding$ACCOUNT_NO),
            unique(asset_mgt_products_raw$ACCOUNT_NO))
)
```
