...
```{r}
sd_ibcf = Recommender(data = products_rating_matrix, method = "IBCF")
sd_pop = Recommender(data = products_rating_matrix, method = "POPULAR")
sd_hyb <- HybridRecommender(sd_ibcf, sd_pop, weights = c(0.9,0.1))
...
```{r}
sd_pop_mat[is.na(sd_pop_mat)]=0
sd_ibcf_mat[is.na(sd_ibcf_mat)]=0


sd_hyb_mat = (sd_ibcf_mat*0.9) + (sd_pop_mat*0.1)
```
...
readRDS(file = "../../../Data Prep/SD/04. Extract additional customer details/outputs/product_businesslines.rds")

product_businesslines
```

```{r}
sd_recommendations_proc <-
sd_recommendations %>%
...
```{r}
product_values <-
readRDS(file = "../../../Data Prep/SD/04. Extract additional customer details/outputs/product_values.rds")

product_values
```

```{r}
sd_recommendations_df_proc <-
sd_recommendations_df %>% 
...
readRDS(file = "../../../Data Prep/SD/04. Extract additional customer details/outputs/customer_details_final.rds") %>% 
  ungroup()
...
```{r}
sd_sqlite_path <- "../../../App/uap_cross_sell/db/sd_cs.db"
con_sd <- dbConnect(drv = SQLite(), sd_sqlite_path)
```


```{r}
dbListTables(conn = con_sd)
```
...
```{r}
copy_to(dest = con_sd, 
        df = sd_recommendations_detailed, 
...
```{r}
dim(sd_recommendations_detailed)

tbl(src = con_sd, "recommendations_detailed") %>% 
  collect() %>% 
  dim()
```
...
```{r}
copy_to(dest = con_sd, df = popular_products, overwrite = TRUE, temporary = FALSE)
...
```{r}
tbl(src = con_sd, "popular_products") %>%
  group_by(BUSINESS_LINE) %>% 
  collect() %>% 
  slice(1:3)
```
...
```{r}
# Project image
save.image(file = "../outputs/23June_Models.RData")
# load(file = "../outputs/23June_Models.RData")

# Ojects for deployment
saveRDS(object = sd_rec_model, 
        file = "../../../App/uap_cross_sell/objects/models/Sudan.rds")

saveRDS(object = sd_recommendations_detailed,
        file = "../outputs/sd_recommendations_detailed.rds")
```
