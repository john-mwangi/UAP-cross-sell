...
```{r}
gh_ibcf = Recommender(data = products_rating_matrix, method = "IBCF")
gh_pop = Recommender(data = products_rating_matrix, method = "POPULAR")
gh_hyb <- HybridRecommender(gh_ibcf, gh_pop, weights = c(0.9,0.1))
...
```{r}
gh_pop_mat[is.na(gh_pop_mat)]=0
gh_ibcf_mat[is.na(gh_ibcf_mat)]=0


gh_hyb_mat = (gh_ibcf_mat*0.9) + (gh_pop_mat*0.1)
```
...
readRDS(file = "../../../Data Prep/GH/03. Extract additional customer details/outputs/product_businesslines.rds")

product_businesslines
```

```{r}
gh_recommendations_proc <-
gh_recommendations %>%
...
```{r}
product_values <-
readRDS(file = "../../../Data Prep/GH/03. Extract additional customer details/outputs/product_values.rds")

product_values
```

```{r}
gh_recommendations_df_proc <-
gh_recommendations_df %>% 
...
readRDS(file = "../../../Data Prep/GH/03. Extract additional customer details/outputs/customer_details_final.rds") %>% 
  ungroup()
...
```{r}
gh_sqlite_path <- "../../../App/uap_cross_sell/db/gh_cs.db"
con_gh <- dbConnect(drv = SQLite(), gh_sqlite_path)
```


```{r}
dbListTables(conn = con_gh)
```
...
```{r}
copy_to(dest = con_gh, 
        df = gh_recommendations_detailed, 
...
```{r}
dim(gh_recommendations_detailed)

tbl(src = con_gh, "recommendations_detailed") %>% 
  collect() %>% 
  dim()
```
...
```{r}
gh_recommendations_detailed %>% 
  filter(intermediated=="Yes") %>% 
  filter(ownership=="Life") %>% 
  filter(product_value>=50) %>% 
  filter(product_value<=200) %>% 
  slice(1:3)
 #head(1000)
```
...
```{r}
copy_to(dest = con_gh, df = popular_products, overwrite = TRUE, temporary = FALSE)
...
```{r}
tbl(src = con_gh, "popular_products") %>%
  group_by(BUSINESS_LINE) %>% 
  collect() %>% 
  slice(1:3)
```
...
```{r}
# Project image
save.image(file = "../10June_Models.RData")
#load(file = "../outputs/16May_Models.RData")

# Ojects for deployment
saveRDS(object = gh_rec_model, 
        file = "../../../App/uap_cross_sell/objects/models/Ghana.rds")

saveRDS(object = gh_recommendations_detailed,
        file = "../gh_recommendations_detailed.rds")
```
