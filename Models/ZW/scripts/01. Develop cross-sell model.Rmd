...
input_account_num <- str_remove_all(string = input_account_num, 
                                    pattern = " ")
input_account_num <- str_split(string = input_account_num, pattern = ",")[[1]]

test_acc <- 30105335
test_stmt <-
glue::glue_sql("
               SELECT * 
               FROM uniqueid_accounts 
               WHERE ACCOUNT_NO like '%{test_acc}%';
               ", .con = con_zw)
...
for(account in seq_along(input_account_num[1])){
  query_result <- 
  dbSendStatement(conn = con_zw, 
                  statement = test_stmt)
  
  res_tbl <- dbFetch(res = query_result)
...
cust_unique_ids <-
tbl(src = con_zw, "uniqueid_accounts") %>% 
  filter(str_detect(string = ACCOUNT_NO, 
                    pattern = convert_nums_to_pattern(
                      account_num = input_account_num)
                    )) %>% 
...
```{r}
# Project image
save.image(file = "../outputs/10June_Models.RData")
#load(file = "../outputs/09June_Models.RData")
