...
zw_recommendations_unique_id %>% 
  left_join(accounts, by = "UNIQUE_ID") %>%
...
Customers with multiple Account Numbers.
...
```{r}
zw_recommendations_detailed %>% 
  filter(str_detect(string = ACCOUNT_NO, pattern = "41398995"))
```
...
```{r}
# Project image
save.image(file = "../outputs/09June_Models.RData")
