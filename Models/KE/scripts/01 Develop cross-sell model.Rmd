...
Despite best efforts including running the hybrid on Google Colab, the model could not complete evaluation As a workaround, we trained a hybrid recommender with 90% IBCF and 10% POP knowing that IBCF is the better performing of the two models. (see Scenario 1).
...
ke_hyb <- HybridRecommender(ke_ibcf, ke_pop, weights = c(0.9, 0.1))

ke_rec_model <- HybridRecommender(ke_ibcf, ke_pop, weights = c(0.9, 0.1))
...
User ratings represent how likely a user is to purchase a product. This approach took too long.
...
}, title = "Hybrid ratings")
```


### Approach 2

Do a join of the tables.

```{r}
ibcf_ke_recommendations <-
predict(object = ke_ibcf, 
...
ibcf_ke_recommendations_df
```
A left join on the basis of user and item cannot be done because 17M are too many records to reference. The task is taking too long to complete.

```{r}
pop_ke_recommendations_df <-
pop_ke_recommendations %>% 
  as("data.frame") %>% 
  as_tibble()
...
```{r}
identical(x = colnames(ibcf_ke_recommendations), 
          y = colnames(pop_ke_recommendations))

identical(x = rownames(ibcf_ke_recommendations), 
          y = rownames(pop_ke_recommendations))
```

Obtain hybrid ratings...

```{r}
ibcf_ke_matrix <- as(object = ibcf_ke_recommendations, Class = "matrix") * 0.9
pop_ke_matrix <- as(object = pop_ke_recommendations, Class = "matrix") * 0.1


hyb_ke_matrix <- ibcf_ke_matrix + pop_ke_matrix
```


```{r}
ke_recommendations_raw <-
hyb_ke_matrix %>% 
  as.data.frame() %>% 
...
Limit: Since there are >17M recommendations, we retain only the top 5 products per customer per business line.
...
```{r}
ke_recommendations_df %>% 
  filter(ACCOUNT_NO %in% c("1337","1418"))
```

Comparison...

```{r}
predict(object = ke_hyb, 
        newdata = products_rating_matrix[1],
        type = "ratings") %>% 
  as("data.frame") %>% 
...
```{r}
ke_recommendations_detailed <- readRDS(file = "../../../Data Prep/KE/03. Extract additional customer details/outputs/ke_recommendations_detailed.rds")
```
...
Check if all business lines are represented in the IBCF model.
...
Compare with popular recommender. 
...
We will create a hybrid recommender with 90% IBCF and 10% POP recommenders.
...
Faster way of extracting ratings.
...
predict(object = ke_pop, 
  ...
  left_join(
    predict(object = ke_ibcf, 
      ...
      as("data.frame"), 
    by = c("user","item")) %>% 
  rename(ibcf_rating = rating.x,
         pop_rating = rating.y) %>% 
  mutate(hyb_rating = 0.9*ibcf_rating + 0.1*pop_rating) %>% 
  left_join(product_businesslines, by = c("item"="PRODUCT")) %>% 
  group_by(user, BUSINESS_LINE) %>% 
  slice(1:3)
```
...
# Project image
save.image(file = "./images/04May_Models.RData")

# Image containing trained model
load("./images/04May_Models.RData")

# Ratings
saveRDS(object = ke_recommendations_df, 
        file = "../outputs/ratings/ke_recommendations_df.rds")
