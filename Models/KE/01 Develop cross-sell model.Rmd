...
```{r}
# Sample of 500 records for display
...
dim(recommendations_df_sample)
head(recommendations_df_sample)
```
...
```{r}
acc_nums_upload <- readxl::read_excel("./cs_upload_template.xlsx", sheet = "customer_acc") %>% pull(ACCOUNT_NO)

cust_prods_upload <- readxl::read_excel("./cs_upload_template.xlsx", sheet = "customer_prod") %>% pull(PRODUCT)
```
...
# Project image
save.image(file = "./07Apr_Models.RData")
