...
```{r}
ng_ownership <- readRDS(file = "../../NG/outputs/ng_recommendations_detailed.rds") %>% 
  ungroup() %>% 
  distinct(ownership) %>% 
  pull(ownership)
...
```{r}
path_ug_supp <- "../../../App/uap_cross_sell/db/support_ug.db"
con_supp_ug <- dbConnect(drv = RSQLite::SQLite(), path_ug_supp)

ug_ownership <- tbl(src = con_supp_ug, "support") %>% 
  filter(country == "Uganda") %>% 
  filter(!is.na(ownership)) %>% 
  pull(ownership)
...
```{r}
ng_products <- readRDS(file = "../../../Data Prep/NG/03. Create rating matrices/outputs/products_rating_matrix.rds") %>% 
  colnames()
...
```{r}
ug_products <- tbl(src = con_supp_ug, "support") %>% 
  filter(country == "Uganda") %>% 
...
ng_tibble <- tibble(
    ...
    ownership = c(ng_ownership,
                  rep(NA, times = length(ng_products) - length(ng_ownership))
                  )
    )
...
```{r}
rbind(support_df, ng_tibble, ug_tibble) %>% 
  distinct(country)
```
...
```{r}
copy_to(dest = con_supp, 
        df = rbind(support_df, ng_tibble, ug_tibble), 
        name = "support", 
        overwrite = TRUE, 
        temporary = FALSE)
```
