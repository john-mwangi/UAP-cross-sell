...
```{r}
ng_ibcf = Recommender(data = products_rating_matrix, method = "IBCF")
ng_pop = Recommender(data = products_rating_matrix, method = "POPULAR")
ng_hyb <- HybridRecommender(ng_ibcf, ng_pop, weights = c(0.9,0.1))

# IBCF is not recommending General Products; hybrid recommender will be used.
ng_rec_model <- ng_hyb
```
...
```{r}
ng_pop_mat[is.na(ng_pop_mat)]=0
ng_ibcf_mat[is.na(ng_ibcf_mat)]=0


ng_hyb_mat = (ng_ibcf_mat*0.9) + (ng_pop_mat*0.1)
```
...
readRDS(file = "../../../Data Prep/NG/04. Extract additional customer details/outputs/product_businesslines.rds")

product_businesslines
```

```{r}
ng_recommendations_proc <-
ng_recommendations %>%
...
```{r}
product_values <-
readRDS(file = "../../../Data Prep/NG/04. Extract additional customer details/outputs/product_values.rds")

product_values
```

```{r}
ng_recommendations_df_proc <-
ng_recommendations_df %>% 
...
readRDS(file = "../../../Data Prep/NG/04. Extract additional customer details/outputs/customer_details_final.rds") %>% 
  ungroup()
...
Based on feedback from Rachel and `cokey@oldmutualnigeria.com` of Nigeria, data from OIPA and GIBBS should be excluded because OIPA has test data and GIBBS is no longer in use.

```{r eval=FALSE, include=FALSE}
ng_recommendations_detailed_filtered <-
ng_recommendations_detailed %>% 
...
```{r}
copy_to(dest = con_ng, 
        df = ng_recommendations_detailed, 
...
```{r}
dim(ng_recommendations_detailed)
...
records <- sample(x = rownames(products_rating_matrix),
                  size = 2)
...
The hybrid recommender does not weight the popular recommendations by 0.1
...
```{r}
copy_to(dest = con_ng, df = popular_products, overwrite = TRUE, temporary = FALSE)
...
```{r}
tbl(src = con_ng, "popular_products") %>%
  group_by(BUSINESS_LINE) %>% 
  collect() %>% 
  slice(1:3)
```
...
```{r}
# Project image
save.image(file = "../outputs/30June_Models.RData")
#load(file = "../outputs/03June_Models.RData")

# Ojects for deployment
saveRDS(object = ng_rec_model, 
        file = "../../../App/uap_cross_sell/objects/models/Nigeria.rds")

saveRDS(object = ng_recommendations_detailed,
        file = "../outputs/ng_recommendations_detailed.rds")
```
