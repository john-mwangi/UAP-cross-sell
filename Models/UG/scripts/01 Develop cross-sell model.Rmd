...
```{r}
ug_rec_model = Recommender(data = products_rating_matrix, method = "IBCF")
...
```{r}
ug_recommendations_df <- as(ug_recommendations, "data.frame")

ug_recommendations_df <- ug_recommendations_df %>%
...
# Health products
health_products <-
readRDS("../../../Data Prep/UG/product_holding/health_product_holding_final.rds") %>% 
  distinct(Product) %>% 
  pull(Product)

gi_products <-
readRDS(file = "../../../Data Prep/UG/product_holding/gi_product_holding_final.rds") %>% 
...
```{r}
product_values <- readRDS("../../../Data Prep/UG/product_holding/product_values.rds")
...
```{r}
ug_recommendations_detailed <- readRDS(file = "../../../Data Prep/UG/03. Extract additional customer details/outputs/ug_recommendations_detailed.rds")
```
...
tbl(src = con, "ug_recommendations_detailed")
```
...
```{r}
# # Models
saveRDS(object = ug_pop, file = "./ke_pop.rds")
